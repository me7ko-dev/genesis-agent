...
_ROTATION = {"offset": 0}  # ротира началния модел, за да не удря само първия
...
class Brain:
    ...
    def complete(self, messages: List[Dict], tools: List[Dict] | None = None):
        ...
        n = len(ordered_chain)
        if n > 0:
            # Ротираме началото → не удряме винаги само първия (голям) модел.
            start = _ROTATION["offset"] % n
            for round_i in range(RETRY_ROUNDS):
                for step in range(n):
                    attempt = ordered_chain[(start + step) % n]
                    ...
                    try:
                        ...
                        # Следващата мисия да започне от СЛЕДВАЩИЯ модел (разпределя товара).
                        _ROTATION["offset"] = (start + step + 1) % n
